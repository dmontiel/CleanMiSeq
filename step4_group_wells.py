...
df = pd.read_csv(fin,sep = ' ',names = ['headers','seq','well','size'])
...
#					tmpcnt+=1
#					tmpsz +=szlist[jj]
					otudict[hlist[ii]] = np.append(otudict[hlist[ii]],wlist[jj])
		for mm in np.arange(len(otudict[hlist[ii]])):
			print hlist[ii], otudict[hlist[ii]][mm]
#		print otudict[hlist[ii]]
#		if tmpcnt >1:
